...
#define Sonar1limit 50                                                              // см, если обнаружена дистанция меньше, чем это число, то сонар считается сработавшим
#define Sonar2limit 50                                                              // см, если обнаружена дистанция меньше, чем это число, то сонар считается сработавшим
...
float sound_speed;                                                                  // скорость звука в зависимости от температуры, см/мкс
long event_temp_read;
...
void setup() {  
  Wire.begin();
  long event_bme_begin = millis();
  while(!bme.begin() && millis() - event_bme_begin < 5000);                         // ожидание подключения BME/BMP280
  temp_read();
...
// - определение скорости звука в зависимости от температуры воздуха
  if((millis() - event_temp_read) >= 60000) temp_read(); 
...
// -- SRF05 --
  if (!SensorPIR) {                													                        // сканирование пространства перед сонаром 1
    ...
    byte distance = pulseIn(Sonar1echo, HIGH, 5000) * sound_speed / 2.0 ;           // расчет дистанции до препятствия (5000 - таймаут, то есть не ждать сигнала более 5мс) 
                                                        
    if ((distance > 0) && (distance <= Sonar1limit)) {
      Sensor1_Ct = Sensor1_Ct + 1;                                                  // SRF05 : сонар 1 зафиксировал присутствие -> увеличить счетчик    
      
      Serial.print("Bottom Sonar дистанция = ");   
...
// - проверить состояние "ВЕРХНЕГО" сенсора 
  ...
  if (!SensorPIR) {                													                        // сканирование пространства перед сонаром 2
    ...
    delayMicroseconds(10);
    digitalWrite(Sonar2trig, LOW);
    //byte distance = pulseIn(Sonar2echo, HIGH, 5000) / 58.0 ;                        // расчет дистанции до препятствия (5000 - таймаут, то есть не ждать сигнала более 5мс) 
    byte distance = pulseIn(Sonar2echo, HIGH, 5000) * sound_speed / 2.0 ;           // расчет дистанции до препятствия (5000 - таймаут, то есть не ждать сигнала более 5мс) 

    if ((distance > 0) && (distance <= Sonar1limit)) {
      Sensor2_Ct = Sensor2_Ct + 1;                                                  // SRF05 : сонар 1 зафиксировал присутствие -> увеличить счетчик 
         
      Serial.print("Top Sonar дистанция = ");   
      Serial.print(distance); 
      Serial.println(" см"); 
...
// -- PIR -- 
    ...
    if (digitalRead(PIR2out) == LOW) Sensor2_Ct = Sensor2_Ct + 2;                   // PIR : сонар 2 зафиксировал присутствие -> увеличить счетчик
...
// ===== ОПРОС ДАТЧИКА BME280. РАСЧЕТ СКОРОСТИ ЗВУКА =====
void temp_read(){
  event_temp_read = millis();
  if (bme.begin()) {
    Serial.println("BME/BMP280 подключен");    
    ...
    Serial.print("- температура воздуха, *C = ");
    Serial.println(air_temp);
  }
  else air_temp = 25.0;
  
  sound_speed = 0.03311 * (1 + air_temp / 273.0);

  Serial.print("- скорость звука, м/c = "); 
